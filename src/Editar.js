...
const Editar = () => {
  ...
  useEffect(() => {
    ...
    const colect = route.params.colect;

    setTitulo(colect.titulo);
    setGenero(colect.genero);
    setDesenvolvedor(colect.desenvolvedor);
    setConsole(colect.console);
    setLancamento(colect.lancamento);
    setImg(colect.img);
    setId(colect.id);
  }, [])
  ...
  const saveProduct = () => {
    ...
    if (titulo.trim() === "") {
      ...
      Axios.patch("http://10.0.2.2:3000/colections/" + id, {
        titulo,
        genero,
        desenvolvedor,
        console,
  ...
  return (
    ...
    <View style={styles.background}>
      ...
      <TextInput
        value={titulo}
        onChangeText={(txt) => setTitulo(txt)} placeholder="Titulo" style={styles.textInput} />
      ...
      <View style={styles.box}>
        <Button title="Atualizar" onPress={saveProduct} />
